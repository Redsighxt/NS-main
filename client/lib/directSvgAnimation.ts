...
export async function animateElementsDirectlyWithVirtualPages(
  ...
  options: {
    ...
    onComplete?: () => void;
    mode?: "chronological" | "layer"; // New option for virtual page handling
    showPageTransitions?: boolean;
    transitionDuration?: number;
  } = {},
): Promise<void> {
  // Import virtual pages manager
  const { virtualPagesManager } = await import("./virtualPagesManager");

  const mode = options.mode || "chronological";
  const showTransitions = options.showPageTransitions ?? true;
  const transitionDuration = options.transitionDuration || 1000;

  console.log("animateElementsDirectlyWithVirtualPages called with:", {
    elementCount: elements.length,
    container: container,
    mode: mode,
    options: options,
  ...
  if (mode === "chronological") {
    // Chronological mode: animate elements in timeline order with page transitions
    await animateChronologicalWithPages(
      elements,
      container,
      config,
    ...
    // Layer mode: group by pages, animate all elements in each page
    await animateLayerWithPages(
      elements,
      container,
      config,
      options.extendedConfig,
...
// Chronological animation with virtual page transitions
async function animateChronologicalWithPages(
  elements: DrawingElement[],
  container: HTMLElement,
  config: AnimationConfig,
  extendedConfig?: ExtendedAnimationConfig,
  ...
  // Build timeline with page switches
  ...
  for (const item of timeline) {
    if (item.type === "page-switch" && showTransitions) {
      // Show page transition
      await showVirtualPageTransition(container, item.page, transitionDuration);
    } else if (item.type === "element" && item.element) {
      // Animate single element with progressive fills
      await animateDrawingElements([item.element], container, {
        ...config,
        delay: 0, // No delay between single elements
      });
    ...
    processedItems++;
    if (onProgress) {
      onProgress((processedItems / timeline.length) * 100);
    }
  }
}

// Layer animation with virtual page grouping
async function animateLayerWithPages(
  elements: DrawingElement[],
  container: HTMLElement,
  config: AnimationConfig,
  extendedConfig?: ExtendedAnimationConfig,
  ...
  const pageGroups = new Map<string, DrawingElement[]>();
  ...
  elements.forEach((element) => {
    ...
    pageGroups.get(pageKey)!.push(element);
  });

  // Sort page groups by first element timestamp
  const sortedPageGroups = Array.from(pageGroups.entries())
    .map(([pageId, pageElements]) => ({
      pageId,
      elements: pageElements,
      firstTimestamp: Math.min(...pageElements.map(e => e.timestamp)),
    }))
    .sort((a, b) => a.firstTimestamp - b.firstTimestamp);

  let processedGroups = 0;

  for (const group of sortedPageGroups) {
    if (showTransitions && processedGroups > 0) {
      // Show page transition (except for first page)
      const page = virtualPagesManager.getPage(group.pageId);
      if (page) {
        await showVirtualPageTransition(container, page, transitionDuration);
      }
    }

    // Animate all elements in this page with progressive fills
    await animateDrawingElements(group.elements, container, config);

    processedGroups++;
    if (onProgress) {
      onProgress((processedGroups / sortedPageGroups.length) * 100);
    }
  }
}

// Show virtual page transition effect
async function showVirtualPageTransition(
  ...
  transition.style.transition = `opacity ${duration / 2}ms ease-in-out`;
  ...
  indicator.textContent = page.isOrigin
    ? "📍 Origin Page (0, 0)"
