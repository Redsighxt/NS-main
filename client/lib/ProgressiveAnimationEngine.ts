...
export class ProgressiveAnimationEngine {
  ...
  buildTimeline(elements: DrawingElement[]): AnimationTimeline {
    ...
    console.log(
      `Timeline built: ${keyframes.length} keyframes, ${currentTime}ms total duration`,
    );
  ...
  private animate = (): void => {
    ...
    const globalProgress = Math.min(
      currentTime / this.timeline.totalDuration,
      1,
    );
  ...
  private calculateTrueSpeedDuration(element: DrawingElement): number {
    ...
    const duration =
      (Math.max(totalLength, 10) / this.settings.penStrokes.trueSpeedRate) *
      1000;
  ...
  seekTo(timeMs: number): void {
    ...
    const clampedTime = Math.max(
      0,
      Math.min(timeMs, this.timeline.totalDuration),
    );
    ...
    console.log(
      `Seeked to ${clampedTime}ms (${(globalProgress * 100).toFixed(1)}%)`,
    );
  }
